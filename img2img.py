...
#TODO update naming convention to reflect the controlnet agnostic nature of the config system (pipeline_config instead of controlnet_config for example)
    ...
    def update_ipadapter_style_image_b(self, style_image: Image.Image) -> bool:
        """Update IPAdapter blend slot B style image."""
        if not self.has_ipadapter:
            return False
        try:
            inner = self.stream.stream if hasattr(self.stream, 'stream') else self.stream
            blend_key = getattr(inner, '_ipadapter_blend_key', 'ipadapter_main_blend')
            inner._param_updater.update_style_image(blend_key, style_image, is_stream=False)
            return True
        except Exception:
            return False

    def update_ipadapter_blend_weight(self, t: float) -> bool:
        """Set blend weight between style image A and B (0.0 = only A, 1.0 = only B)."""
        if not self.has_ipadapter:
            return False
        try:
            inner = self.stream.stream if hasattr(self.stream, 'stream') else self.stream
            pu = inner._param_updater
            pu.set_ipadapter_blend_weight(float(t))
            return True
        except Exception:
            return False
