...
_DEFAULT_CONFIG_PATH = str(Path(__file__).parent / "configs" / "NewStart.yaml")
parser.add_argument(
    "--config",
    dest="config",
    type=str,
    default=_DEFAULT_CONFIG_PATH if os.path.exists(_DEFAULT_CONFIG_PATH) else None,
    help="Path to YAML config file to load on startup (defaults to NewStart.yaml if it exists)",
)
