<script lang="ts">
  ...
  // When resetKey changes (e.g. after a snapshot restore), clear preprocessor state
  // so the reactive block below re-fetches current params from the backend.
  let lastResetKey = 0;
    ...
    {#if showControlNet}
      <div class="p-3">
        ...
        {:else if controlnetInfo?.enabled}
          <div class="space-y-2">
            <p class="text-xs text-gray-600 dark:text-gray-400">No ControlNets active. Add one:</p>
            <ControlNetSelector on:controlnetAdded={handleControlNetAdded} />
          </div>
        {:else}
          <div class="space-y-2">
            <p class="text-xs text-gray-600 dark:text-gray-400">Load a config to enable ControlNet.</p>
            <ControlNetSelector on:controlnetAdded={handleControlNetAdded} />
          </div>
        {/if}
      </div>
      ...
      <div class="p-2">
        <div class="space-y-1">
          {#each tIndexList as tIndex, index}
            <div class="space-y-1">
              <div class="flex items-center justify-between">
                <label class="text-xs font-medium text-gray-600 dark:text-gray-400" title="Denoising timestep index (0–49). Higher = more creative, lower = closer to input.">Step {index + 1}</label>
    ...
    {#if showStreamingParams}
      <div class="p-2">
        <div class="space-y-2">
          ...
          <div class="space-y-1">
            <div class="flex items-center justify-between">
              <label class="text-xs font-medium text-gray-600 dark:text-gray-400" title="CFG guidance strength — how strongly the prompt steers the output">Guidance Scale</label>
          ...
          <div class="space-y-1">
            <div class="flex items-center justify-between">
              <label class="text-xs font-medium text-gray-600 dark:text-gray-400" title="Virtual residual noise multiplier">Delta</label>
          ...
          <div class="space-y-1">
            <div class="flex items-center justify-between">
              <label class="text-xs font-medium text-gray-600 dark:text-gray-400" title="Number of denoising steps per frame">Inference Steps</label>
