<script lang="ts">
  ...
  let inputControlConfigs: Array<{
    ...
    axis_index?: number;
    deadzone?: number;
    // Breath-control fields
    breath_signal?: 'amplitude' | 'bpm';
    breath_status?: string;
    breath_last_event?: string;
    // Calibration fields (mic + breath)
    raw_level?: number;
    cal_min?: number;
    cal_max?: number;
    cal_active?: boolean;
    cal_floor_active?: boolean;
    _floor_peak?: number;
    cal_timer?: ReturnType<typeof setTimeout>;
  }> = [];
  ...
  async function updateParameterOptions(settings: any, blendingData: any = null) {
    ...
    if (settings.ipadapter) {
      ...
      parameters.push({
        value: 'ipadapter_blend_weight',
        label: 'Style Image A→B Blend',
        min: 0.0,
        max: 1.0,
        category: 'ipadapter'
      });
    ...
    // Prompt blending — when exactly 2 prompts, expose a single blend slider
    if (Array.isArray(promptBlending) && promptBlending.length === 2) {
      parameters.push({
        value: 'prompt_blend',
        label: 'Prompt A→B Blend',
        min: 0.0,
        max: 1.0,
        category: 'prompt_blending'
      });
    } else if (Array.isArray(promptBlending) && promptBlending.length > 2) {
    ...
    // Seed blending — when exactly 2 seeds, expose a single blend slider
    if (Array.isArray(seedBlending) && seedBlending.length === 2) {
      parameters.push({
        value: 'seed_blend',
        label: 'Seed A→B Blend',
        min: 0.0,
        max: 1.0,
        category: 'seed_blending'
      });
    } else if (Array.isArray(seedBlending) && seedBlending.length > 2) {
  ...
  async function addInputControl(type: 'microphone' | 'hand_tracking' | 'gamepad' | 'breath') {
    ...
    try {
      ...
      const newControl = {
        ...
        is_active: false,
        current_value: 0,
        intervalId: (type === 'microphone' || type === 'breath') ? 0 : undefined,
        ...
        breath_signal: type === 'breath' ? 'amplitude' as const : undefined,
        breath_status: type === 'breath' ? 'DISCONNECTED' : undefined,
        breath_last_event: type === 'breath' ? '' : undefined,
        raw_level: (type === 'microphone' || type === 'breath') ? 0 : undefined,
        cal_min: (type === 'microphone' || type === 'breath') ? 0 : undefined,
        cal_max: (type === 'microphone' || type === 'breath') ? 1 : undefined,
        cal_active: (type === 'microphone' || type === 'breath') ? false : undefined,
        cal_floor_active: (type === 'microphone' || type === 'breath') ? false : undefined,
        _floor_peak: (type === 'microphone' || type === 'breath') ? 0 : undefined,
      };
  ...
  // ---------------------------------------------------------------------------
  // Calibration helpers — remap observed raw signal range to [0, 1]
  // ---------------------------------------------------------------------------

  function applyCalibration(raw: number, control: any): number {
    const lo = control.cal_min ?? 0;
    const hi = control.cal_max ?? 1;
    const range = hi - lo;
    if (range < 0.001) return raw;
    return Math.max(0, Math.min(1, (raw - lo) / range));
  }

  function startCalibration(control: any) {
    if (control.cal_timer) clearTimeout(control.cal_timer);
    control.cal_min = 1.0;
    control.cal_max = 0.0;
    control.cal_active = true;
    inputControlConfigs = [...inputControlConfigs];
    control.cal_timer = setTimeout(() => {
      control.cal_active = false;
      // If nothing was captured, fall back to full range
      if ((control.cal_max ?? 0) <= (control.cal_min ?? 1)) {
        control.cal_min = 0;
        control.cal_max = 1;
      }
      inputControlConfigs = [...inputControlConfigs];
  ...
  function resetCalibration(control: any) {
    if (control.cal_timer) clearTimeout(control.cal_timer);
    control.cal_min = 0;
    control.cal_max = 1;
    control.cal_active = false;
    control.cal_floor_active = false;
    control._floor_peak = 0;
    inputControlConfigs = [...inputControlConfigs];
  }

  // Noise-floor calibration: stay quiet, sample for 3s, peak becomes the new cal_min.
  // Re-run anytime if the first attempt caught extra noise.
  function startFloorCalibration(control: any) {
    if (control.cal_timer) clearTimeout(control.cal_timer);
    control.cal_floor_active = true;
    control._floor_peak = 0;
    inputControlConfigs = [...inputControlConfigs];
    control.cal_timer = setTimeout(() => {
      control.cal_floor_active = false;
      // Add a 15% margin so brief noise spikes don't immediately register as signal
      const floor = Math.min((control._floor_peak ?? 0) * 1.15, (control.cal_max ?? 1) * 0.9);
      control.cal_min = parseFloat(floor.toFixed(4));
      inputControlConfigs = [...inputControlConfigs];
    }, 3000);
  }
  ...
  async function startMicrophoneControl(control: any) {
    ...
    control.intervalId = setInterval(async () => {
      try {
        ...
        control.raw_level = level;
        if (control.cal_active) {
          if (level < (control.cal_min ?? 1)) control.cal_min = level;
          if (level > (control.cal_max ?? 0)) control.cal_max = level;
        }
        if (control.cal_floor_active) {
          if (level > (control._floor_peak ?? 0)) control._floor_peak = level;
        }
        const calibrated = applyCalibration(level, control);
        const sensitiveLevel = Math.min(1.0, calibrated * control.sensitivity);
        const scaledValue = control.min_value + (sensitiveLevel * (control.max_value - control.min_value));

        // Skip backend update if value hasn't changed meaningfully
        const lastSent = control._last_sent_value ?? null;
        if (lastSent !== null && Math.abs(scaledValue - lastSent) < 0.001) {
          control.current_value = scaledValue;
          inputControlConfigs = inputControlConfigs;
          return;
        }
        control._last_sent_value = scaledValue;
        control.current_value = scaledValue;
        inputControlConfigs = inputControlConfigs; // trigger reactivity for value display
        ...
        } else if (control.parameter_name === 'prompt_blend') {
          await updatePromptBlendParameter(scaledValue);
        } else if (control.parameter_name === 'seed_blend') {
          await updateSeedBlendParameter(scaledValue);
  ...
  // ---------------------------------------------------------------------------
  // Breath control — polls /api/breath/status at the configured update_rate,
  // reads the selected signal (amplitude or bpm), applies sensitivity, and
  // sends parameter updates through the same pipeline as microphone control.
  // ---------------------------------------------------------------------------

  async function startBreathControl(control: any) {
    const updateInterval = Math.max(50, control.update_rate * 1000);

    control.intervalId = setInterval(async () => {
      try {
        const response = await fetch('/api/breath/status');
        if (!response.ok) return;
        const data = await response.json();

        // Update display-only state (connection status + last event label)
        control.breath_status = data.status;
        control.breath_last_event = data.last_event;
        inputControlConfigs = inputControlConfigs; // trigger reactivity for status badge

        // Only drive parameter updates when live data is flowing
        if (data.status !== 'LIVE') return;

        // Pick the selected signal value (both are already 0-1 normalised by backend)
        const signalValue: number = control.breath_signal === 'bpm' ? data.bpm : data.amplitude;
        control.raw_level = signalValue;
        if (control.cal_active) {
          if (signalValue < (control.cal_min ?? 1)) control.cal_min = signalValue;
          if (signalValue > (control.cal_max ?? 0)) control.cal_max = signalValue;
        }
        if (control.cal_floor_active) {
          if (signalValue > (control._floor_peak ?? 0)) control._floor_peak = signalValue;
        }
        const calibrated = applyCalibration(signalValue, control);
        const sensitiveLevel = Math.min(1.0, calibrated * control.sensitivity);
        const scaledValue = control.min_value + (sensitiveLevel * (control.max_value - control.min_value));

        // Skip backend update if value hasn't changed meaningfully
        const lastSentB = control._last_sent_value ?? null;
        if (lastSentB !== null && Math.abs(scaledValue - lastSentB) < 0.001) {
          control.current_value = scaledValue;
          return;
        }
        control._last_sent_value = scaledValue;
        ...
        // Send parameter update — same routing logic as microphone/gamepad controls
        if (control.parameter_name.startsWith('controlnet_') && control.parameter_name.endsWith('_strength')) {
          await updateControlNetParameter(control, scaledValue);
        } else if (control.parameter_name === 'prompt_blend') {
          await updatePromptBlendParameter(scaledValue);
          ...
          if (endpoint) {
            await fetch(endpoint, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ weight_type: weightTypes[index] })
            });
          }
        } else {
          const endpoint = getParameterUpdateEndpoint(control.parameter_name);
        ...
        console.error('startBreathControl: Update failed:', error);
      }
    }, updateInterval);

    control.is_active = true;
    inputControlConfigs = [...inputControlConfigs];
    statusMessage = `Started breath control ${control.id}`;
  }

  function stopBreathControl(control: any) {
    if (control.intervalId) {
      clearInterval(control.intervalId);
      control.intervalId = 0;
    }
    control.is_active = false;
    control.breath_status = 'DISCONNECTED';
    control.breath_last_event = '';
    inputControlConfigs = [...inputControlConfigs];
    statusMessage = `Stopped breath control ${control.id}`;
  }

  function stopAllBreathControls() {
    inputControlConfigs.forEach(control => {
      if (control.type === 'breath' && control.intervalId) {
        clearInterval(control.intervalId);
        control.intervalId = 0;
        control.is_active = false;
      }
  ...
  function getParameterUpdateEndpoint(parameterName: string): string | null {
    ...
    const endpoints: Record<string, string> = {
      ...
      'ipadapter_blend_weight': '/api/ipadapter/blend-weight',
  ...
  function getParameterKey(parameterName: string): string {
    ...
    const keys: Record<string, string> = {
      ...
      'ipadapter_blend_weight': 'blend_weight',
  ...
  // Handle prompt A→B blend (single 0-1 slider → computes both weights)
  async function updatePromptBlendParameter(t: number) {
    if (!Array.isArray(promptBlendingConfig) || promptBlendingConfig.length !== 2) return;
    const updated = [
      [promptBlendingConfig[0][0], parseFloat((1 - t).toFixed(3))],
      [promptBlendingConfig[1][0], parseFloat(t.toFixed(3))]
    ];
    await fetch('/api/blending', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ prompt_list: updated, prompt_interpolation_method: 'slerp' })
    }).catch(e => console.error('updatePromptBlendParameter:', e));
  }

  // Handle seed A→B blend (single 0-1 slider → computes both weights)
  async function updateSeedBlendParameter(t: number) {
    if (!Array.isArray(seedBlendingConfig) || seedBlendingConfig.length !== 2) return;
    const updated = [
      [seedBlendingConfig[0][0], parseFloat((1 - t).toFixed(3))],
      [seedBlendingConfig[1][0], parseFloat(t.toFixed(3))]
    ];
    await fetch('/api/blending', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ seed_list: updated, seed_interpolation_method: 'linear' })
    }).catch(e => console.error('updateSeedBlendParameter:', e));
  }
      ...
      if (control.pendingValue !== null && control.pendingValue !== control.lastSentValue) {
        try {
          ...
          if (control.parameter_name.startsWith('controlnet_') && control.parameter_name.endsWith('_strength')) {
            ...
            await updatePromptBlendParameter(control.pendingValue);
          } else if (control.parameter_name === 'seed_blend') {
            await updateSeedBlendParameter(control.pendingValue);
          } else if (control.parameter_name.startsWith('prompt_weight_')) {
  ...
  function removeInputControl(index: number) {
    ...
    if (control.is_active) {
      if (control.type === 'microphone') {
        ...
        control.is_active = false;
      } else if (control.type === 'breath') {
        stopBreathControl(control);
      }
    }
  ...
  function updateControlParameter(index: number, field: string, value: any) {
    ...
    } else if (field === 'breath_signal') {
      control.breath_signal = value;
    }
    ...
    <div class="panel-content">
      ...
      <div class="add-button-section">
        <div class="flex gap-2 flex-wrap">
          ...
          <Button
            on:click={() => addInputControl('breath')}
            disabled={isLoading}
          >
            {isLoading ? 'Adding...' : 'Add Breath Control'}
          </Button>
        </div>
      </div>
            ...
            <div class="control-form">
              <div class="control-header">
                <div class="control-title">
                  ...
                  {#if control.type === 'breath'}
                    <span class="status-badge"
                      class:breath-live={control.breath_status === 'LIVE'}
                      class:breath-waiting={control.breath_status === 'CONNECTED'}
                    >
                      {control.breath_status === 'LIVE' ? 'LIVE' :
                       control.breath_status === 'CONNECTED' ? 'WAITING' : 'DISCONNECTED'}
                    </span>
                    {#if control.breath_last_event}
                      <span class="breath-event-label">
                        {control.breath_last_event === 'inhale' ? '↑ inhale' :
                         control.breath_last_event === 'exhale' ? '↓ exhale' :
                         control.breath_last_event === 'cycle_end' ? '↺ cycle' :
                         control.breath_last_event}
                      </span>
                    {/if}
                  {/if}
                  {#if (control.type === 'microphone' || control.type === 'breath') && control.raw_level !== undefined}
                    <span class="value-display raw-level" title="Raw input level">
                      raw: {control.raw_level.toFixed(3)}
                    </span>
                  {/if}
              ...
              <div class="control-config">
                <div class="form-grid">
                  <div class="form-group">
                   ...
                   {#if control.type === 'microphone' || control.type === 'breath'}
                     <div class="form-group calibration-group">
                       <label>Input Calibration:</label>
                       <div class="cal-controls">
                         <span class="cal-range" title="Observed input range used for scaling">
                           [{(control.cal_min ?? 0).toFixed(3)} – {(control.cal_max ?? 1).toFixed(3)}]
                         </span>
                         <Button
                           on:click={() => startFloorCalibration(control)}
                           disabled={control.cal_floor_active || control.cal_active || !control.is_active}
                           title={!control.is_active ? 'Start the control first' : 'Stay quiet for 3s — sets noise floor as minimum'}
                         >
                           {control.cal_floor_active ? 'Listening…' : 'Noise Floor 3s'}
                         </Button>
                         <Button
                           on:click={() => startCalibration(control)}
                           disabled={control.cal_active || control.cal_floor_active || !control.is_active}
                           title={!control.is_active ? 'Start the control first' : 'Record full input range for 5 seconds'}
                         >
                           {control.cal_active ? 'Calibrating…' : 'Full Range 5s'}
                         </Button>
                         <Button on:click={() => resetCalibration(control)} title="Reset to full [0–1] range">
                           Reset
                         </Button>
                       </div>
                       {#if !control.is_active}
                         <span class="cal-hint">Start control to enable calibration</span>
                       {:else if control.cal_floor_active}
                         <span class="cal-hint cal-recording">Stay quiet — sampling room noise floor…</span>
                       {:else if control.cal_active}
                         <span class="cal-hint cal-recording">Recording — perform your full input range now…</span>
                       {/if}
                     </div>
                   {/if}
                    ...
                    {#if control.type === 'breath'}
                      <div class="form-group">
                        <label>Signal:</label>
                        <select
                          bind:value={control.breath_signal}
                          on:change={(e) => updateControlParameter(index, 'breath_signal', (e.target as HTMLSelectElement).value)}
                        >
               ...
               <!-- Breath section — shown for breath controls -->
               {#if control.type === 'breath'}
                 <div class="breath-section">
                   <div class="breath-info">
                     <span class="text-sm" style="color: #9ca3af;">
                       Connects to breath.py at ws://localhost:8765 —
                       start breath.py with <code>python breath.py --ws</code> before activating.
                     </span>
                   </div>

                   <!-- ═══════════════════════════════════════════════════════════════
                        STEP 2 PLACEHOLDER — Speech → Prompt
                        When breath.py emits "speech" events (Step 2), add a UI section
                        here to configure how transcribed speech maps to prompt changes.
                        The backend BreathInputSource already has a commented _handle_speech
                        hook ready to activate — no structural changes needed there.

                        Planned UI elements:
                          - Enable speech-to-prompt toggle
                          - Mode selector: "append to prompt" | "replace prompt" | "LLM rewrite"
                          - Preview of last transcription
                          - Preview of last prompt change
                        ═══════════════════════════════════════════════════════════════ -->
                 </div>
               {/if}
...
<style>
  ...
  .status-badge.breath-live {
    background: #059669;
  }

  .status-badge.breath-waiting {
    background: #d97706;
  }
