<script lang="ts">
  ...
  // Keep blend sliders in sync when list is loaded from config
  $: if (promptList.length === 2) {
    const total = promptList[0][1] + promptList[1][1];
    promptBlend = total > 0 ? promptList[1][1] / total : 0.5;
  }
  $: if (seedList.length === 2) {
    const total = seedList[0][1] + seedList[1][1];
    seedBlend = total > 0 ? seedList[1][1] / total : 0.5;
  }

  function handlePromptBlendChange(t: number) {
    promptList[0][1] = parseFloat((1 - t).toFixed(3));
    promptList[1][1] = parseFloat(t.toFixed(3));
    promptList = [...promptList];
    updatePromptBlendingWithoutRefresh();
  }

  function handleSeedBlendChange(t: number) {
    seedList[0][1] = parseFloat((1 - t).toFixed(3));
    seedList[1][1] = parseFloat(t.toFixed(3));
    seedList = [...seedList];
    updateSeedBlendingWithoutRefresh();
  }

  // Force re-initialization when resetKey changes (e.g. after params restore).
  // We apply config directly here rather than just clearing promptInitialized,
  // because the hash-change guard in the config reactive blocks re-init when
  // the same config is loaded again (e.g. loading the same snapshot twice).
  let lastResetKey = 0;
  $: if (resetKey !== lastResetKey) {
    lastResetKey = resetKey;
    hasPendingPromptChanges = false;
    ...
    if (promptBlendingConfig && Array.isArray(promptBlendingConfig)) {
      ...
      lastPromptConfigHash = getPromptConfigHash(promptBlendingConfig, currentPrompt);
    } else {
    ...
    if (seedBlendingConfig && Array.isArray(seedBlendingConfig)) {
      ...
      lastSeedConfigHash = getSeedConfigHash(seedBlendingConfig);
    } else {
      ...
      <div class="space-y-2">
        ...
        {#each promptList as [prompt, weight], index (index)}
          <div class="bg-gray-50 dark:bg-gray-700 rounded p-2 space-y-2">
            <div class="flex items-center justify-between">
              <span class="text-xs font-medium">Prompt {index + 1}</span>
              <div class="flex items-center gap-2">
                {#if promptList.length !== 2}
                  <span class="text-xs text-gray-600 dark:text-gray-400">
                    Weight: {promptList[index][1].toFixed(2)}
                  </span>
                {/if}
                {#if promptList.length > 1}
            ...
            <textarea
              ...
              rows="2"
              class="w-full p-2 border rounded resize-none text-sm dark:bg-gray-600 dark:border-gray-500"
            ></textarea>
        ...
        {#if promptList.length === 2}
          ...
          <div class="space-y-1">
            ...
            <input
              ...
              max="1"
              step="0.01"
              bind:value={promptBlend}
              on:input={() => handlePromptBlendChange(promptBlend)}
              class="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer dark:bg-gray-600"
            />
          </div>
        {:else}
          <div class="text-xs text-gray-600 dark:text-gray-400 text-center">
            Total Weight: {promptList.reduce((sum, [, weight]) => sum + weight, 0).toFixed(2)}
          </div>
        {/if}
    ...
    <div class="space-y-4">
      ...
      <div class="space-y-3">
        ...
        {#each seedList as [seed, weight], index}
          <div class="bg-gray-50 dark:bg-gray-700 rounded p-2 space-y-2">
            <div class="flex items-center justify-between">
              <span class="text-xs font-medium">Seed {index + 1}</span>
              <div class="flex items-center gap-2">
                {#if seedList.length !== 2}
                  <span class="text-xs text-gray-600 dark:text-gray-400">
            ...
            <div class="flex items-center gap-2">
              <input
                ...
                min="1"
                max="999999"
                class="flex-1 p-2 border rounded text-sm dark:bg-gray-600 dark:border-gray-500"
              />
        ...
        {#if seedList.length === 2}
          ...
          <div class="space-y-1">
            <div class="flex justify-between text-xs text-gray-500">
              <span>← Seed 1 only</span>
              <span>Seed 2 only →</span>
            </div>
            <input
              type="range"
              min="0"
              max="1"
              step="0.01"
              bind:value={seedBlend}
              on:input={() => handleSeedBlendChange(seedBlend)}
              class="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer dark:bg-gray-600"
            />
          </div>
        {:else}
          <div class="text-xs text-gray-600 dark:text-gray-400 text-center">
            Total Weight: {seedList.reduce((sum, [, weight]) => sum + weight, 0).toFixed(2)}
          </div>
        {/if}
      </div>
    </div>
