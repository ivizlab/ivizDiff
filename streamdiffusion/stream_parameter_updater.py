...
class StreamParameterUpdater(OrchestratorUser):
    def __init__(self, stream_diffusion, wrapper=None, normalize_prompt_weights: bool = True, normalize_seed_weights: bool = True):
        ...
        # Blend weight for A/B image lerp (0.0 = 100% A, 1.0 = 100% B)
        self._ipadapter_blend_weight: float = 0.0
    ...
    def set_ipadapter_blend_weight(self, t: float) -> None:
        """Set blend weight between style image A and B (0.0 = 100% A, 1.0 = 100% B).
        Triggers embedding recomputation so the new blend takes effect immediately.
        """
        self._ipadapter_blend_weight = max(0.0, min(1.0, float(t)))
        # Recompute blended embeddings so the hook runs with the new weight
    ...
    def update_stream_params(
        ...
        with self._update_lock:
            ...
            elif negative_prompt is not None and self._current_prompt_list:
                # Negative prompt changed alone — re-encode with current prompt list
                self._prompt_cache.clear()  # invalidate cache so negative embeds are recomputed
                self._update_blended_prompts(
                    prompt_list=self._current_prompt_list,
            ...
            if ipadapter_config is not None:
                logger.debug(f"update_stream_params: Updating IPAdapter configuration")
