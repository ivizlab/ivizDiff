...
class IPAdapterModule(OrchestratorUser):
    ...
    def build_embedding_hook(self, stream) -> EmbeddingHook:
        ...
        def _embedding_hook(ctx: EmbedsCtx) -> EmbedsCtx:
            # Fetch slot A embeddings
            cached_a: Optional[Tuple[torch.Tensor, torch.Tensor]] = stream._param_updater.get_cached_embeddings(style_key)
            # Fetch slot B embeddings (blend target) — None when no second image loaded
            blend_key = style_key + "_blend"
            cached_b: Optional[Tuple[torch.Tensor, torch.Tensor]] = stream._param_updater.get_cached_embeddings(blend_key)
            t: float = getattr(stream._param_updater, '_ipadapter_blend_weight', 0.0)

            image_prompt_tokens: Optional[torch.Tensor] = None
            image_negative_tokens: Optional[torch.Tensor] = None
            if cached_a is not None:
                img_a, neg_a = cached_a
                if cached_b is not None and t > 0.0:
                    # Lerp in embedding space — zero cost when t==0 or B is absent
                    img_b, neg_b = cached_b
                    image_prompt_tokens = torch.lerp(img_a, img_b, t)
                    image_negative_tokens = torch.lerp(neg_a, neg_b, t) if neg_a is not None and neg_b is not None else neg_a
                else:
                    image_prompt_tokens, image_negative_tokens = img_a, neg_a
    ...
    def install(self, stream) -> None:
        ...
        # Register same preprocessor for blend slot B so second images can be encoded
        blend_key = style_key + "_blend"
        stream._param_updater.register_embedding_preprocessor(embedding_preprocessor, blend_key)
        # Store key names on stream so pipeline code can find them without hard-coding
        stream._ipadapter_main_key = style_key
        stream._ipadapter_blend_key = blend_key
